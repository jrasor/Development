...
public class PIDController {
  ...
  private double m_input;               // sensor input
  private double m_maximumOutput = 1.0;
  private double m_minimumOutput = 0.0; // absolute value of minimum
  private double m_maximumInput = 0.0;  // limit setpoint to ...
  private double m_minimumInput = 0.0;  // ... these
  private boolean m_continuous = false; // allow endpoints wrap around
  // e. g. for an Absolute encoder.
  private boolean m_enabled = false;    // is a pid controller instance enabled
  private double m_prevError = 0.0;     // the prior sensor input. It's used to
  // compute velocity, and so m_D.
  private double m_totalError = 0.0;    // the sum of the errors for use in
  // the integral calc.
  private double m_tolerance = 0.01;    // relative error tolerance
  ...
  private void calculate() {
    ...
    if (m_enabled) {
      ...
      m_result =
              (m_P * m_error) + (m_I * m_totalError) + (m_D * (m_error - m_prevError));
      ...
      // Make sure the final result is within bounds. If we constrain the
      // result, we make
      // sure the sign of the constrained result matches the original result
      // sign.
   ...
   *              This is always centered on zero and constrained the the max
   *              and min outs
   * @return the latest calculated output
   */
   ...
   *
   * @param minimumInput the minimum value expected from the input, always
   *                     positive
   * @param maximumInput the maximum value expected from the output, always
   *                     positive
   */
   ...
   *
   * @param minimumOutput the minimum value to write to the output, always
   *                      positive
   * @param maximumOutput the maximum value to write to the output, always
   *                      positive
   */
  ...
  /**
   * Set the relative error which is considered tolerable for use with
   * OnTarget. Parameter of 0.05 means we're good within 5 percent.
   *
   * @param relative_error error which is tolerable
   */
  public void setTolerance(double relative_error) {
    m_tolerance = relative_error;
  }
  ...
  public boolean onTarget() {
    return (Math.abs(m_error) < Math.abs(m_tolerance * (m_maximumInput - m_minimumInput)));
  }
